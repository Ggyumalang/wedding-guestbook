...
- **예식 PIN 보호 (Authentication)**: 생성된 예식 정보에 PIN 비밀번호를 설정하여, 인가된 접수자만이 예식에 접근하고 데이터를 수정할 수 있도록 보호합니다.
- **스마트 리스트 UI (UI 최적화)**: 입력 페이지 내에서 바로 확인할 수 있는 **최근 접수 내역 접기/펴기(미니 리스트)** 기능을 지원하며, 전체 내역 리스트에서는 **페이지 당 항목 수 커스텀, 페이지별 누적 금액 표시**를 통해 효율적인 데이터 조회를 돕습니다.
...
- **Step 1: 진입 및 예식 선택 (인증)**
    - 앱 실행 $\rightarrow$ 진행할 예식(신랑/신부) 선택 또는 새 예식 생성.
    - 기존 예식 진입 시 **PIN 번호**를 입력하여 안전하게 인증.
...
- **Left Section (고속 1차 입력 폼 및 최근 내역)**
    - **순번:** (Input - Number) *자동 증가 또는 수동 입력*
    - **금액:** (Input - Number) *단축 버튼 제공 (+5만, +10만 등)*
    - **식권 배부:** (Input - Number)
    - **[접수 완료] 버튼** (Enter 매핑)
    - **미니 최근 리스트:** 접히고 펼쳐지는 컴포넌트로 방금 입력한 항목들을 빠르게 모니터링.
